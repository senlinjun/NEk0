use crate::{TsChannel, TsClient, TsEvent, Command, STATE, RUNTIME, COMMAND_TX};
...
use tsclientlib::{Connection, DisconnectOptions, StreamItem, OutCommandExt};
use tsclientlib::messages::c2s::*;
use tsclientlib::{ChannelId, ClientId};
use tsproto_packets::packets::{AudioData, CodecType, OutAudio};

fn to_c_str(s: String) -> *mut c_char {
    CString::new(s).unwrap_or_else(|_| CString::new("null string").unwrap())
        .into_raw()
}
...
fn refresh_from_book(book: &tsclientlib::data::Connection) -> (Vec<TsChannel>, Vec<TsClient>) {
    ...
    let channels = book.channels.values().map(|c| TsChannel {
        id: c.id.0 as u32,
        name: c.name.clone(),
        parent_id: if c.parent.0 == 0 { 0 } else { c.parent.0 as u32 },
    ...
    let clients = book.clients.values().map(|c| TsClient {
        id: c.id.0 as u32,
...
pub extern "C" fn ts_connect(
    address: *const c_char, nickname: *const c_char,
    channel: *const c_char, password: *const c_char,
) -> *mut c_char {
    let address = unsafe { std::ffi::CStr::from_ptr(address) }.to_string_lossy().into_owned();
    let nickname = unsafe { std::ffi::CStr::from_ptr(nickname) }.to_string_lossy().into_owned();
    let channel = if channel.is_null() { None } else { Some(unsafe { std::ffi::CStr::from_ptr(channel) }.to_string_lossy().into_owned()) };
    let password = if password.is_null() { None } else { Some(unsafe { std::ffi::CStr::from_ptr(password) }.to_string_lossy().into_owned()) };
...

    {
        ...
        if state.connecting || state.connected {
            return to_c_str(serde_json::to_string(&TsEvent::Error { message: "Already connecting".into() }).unwrap());
        }
    ...
    RUNTIME.spawn(async move {
        if let Err(e) = do_connect(address, nickname, channel, password).await {
            ...
            state.pending_events.push_back(TsEvent::Error { message: format!("{}", e) });
        }
    });
...
async fn do_connect(
    ...
    channel: Option<String>, password: Option<String>,
) -> Result<(), String> {
    crate::install_panic_hook();
    let mut opts = Connection::build(address).name(nickname);
    if let Some(ch) = channel { opts = opts.channel(ch); }
    if let Some(pw) = password { opts = opts.password(pw); }
    ...
    tokio::time::timeout(Duration::from_secs(15), async {
        while let Some(item) = con.events().next().await {
            match item {
                Ok(StreamItem::BookEvents(_)) => { ok = true; break; }
    ...
    if !ok { return Err("No BookEvents".into()); }
    ...
    eprintln!("do_connect: OK, {} channels, {} clients, own_id={}", channels.len(), clients.len(), oid);
    ...
    *COMMAND_TX.lock() = Some(cmd_tx);

    {
        ...
        state.pending_events.push_back(TsEvent::Connected { server_name: sname, client_id: oid });
    }
    ...
    *crate::CONNECTION_STASH.lock() = Some(con);
    ...
    RUNTIME.spawn(async move {
        let con = crate::CONNECTION_STASH.lock().take()
...
async fn event_loop(mut con: Connection, mut cmd_rx: tokio::sync::mpsc::UnboundedReceiver<Command>, generation: u64) {
    ...
    loop {
        ...
        if do_disconnect {
            ...
            for i in 0..15 {
                let result = tokio::time::timeout(
                    Duration::from_millis(200), con.events().next()).await;
                match result {
                    Ok(Some(Ok(_))) => {},
                    ...
                    Err(_) => {
                        if i == 14 { push_diag("event_loop: quit wait timeout, exiting"); }
                    }
                }
            }
            ...
            if current_gen == generation {
                ...
                *COMMAND_TX.lock() = None;
                ...
                push_diag(&format!("event_loop: stale disconnect skipped (gen mismatch)"));
            }
            return;
        }
        ...
        while let Ok(cmd) = cmd_rx.try_recv() {
            ...
            match cmd {
                Command::SendMessage { target_mode: _, target_cid: _, message } => {
                    let part = OutSendTextMessagePart {
                        target: tsclientlib::TextMessageTargetMode::Channel,
                        target_client_id: None,
                        message: Cow::Owned(message),
                    };
                    let _ = OutSendTextMessageMessage::new(&mut std::iter::once(part)).send(&mut con);
                    eprintln!("event_loop: sent message");
                }
                Command::MoveChannel { client_id, channel_id } => {
                    ...
                    eprintln!("event_loop: moved client {} to channel {}", client_id, channel_id);
                }
                ...
                Command::SendAudio { data } => {
                    ...
                    loop {
                        let encode_result = {
                            let mut state = STATE.lock();
                            if state.pcm_in.len() < FRAME { break; }
                            ...
                            if let Some(ref mut encoder) = state.audio_encoder {
                                ...
                                match encoder.encode(&frame, FRAME, &mut opus_out) {
                                    Ok(len) => {
                                        let hex_first: String = opus_out[..len.min(8)].iter()
                                            .map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join("");
                                        eprintln!("opus encode: {} samp -> {} bytes [{}...]",
                                            frame.len(), len, hex_first);
                                        let seq = state.audio_seq;
                                        state.audio_seq = state.audio_seq.wrapping_add(1);
                                        Some((seq, opus_out[..len].to_vec()))
                                    }
                                    Err(e) => {
                                        eprintln!("opus encode ERROR: {} (frame_len={})", e, frame.len());
                                        None
                                    }
                                }
                            } else {
        ...
        match result {
            Ok(Some(Ok(item))) => {
                ...
                if let StreamItem::Audio(packet) = &item {
                    ...
                    if codec != CodecType::OpusVoice && codec != CodecType::OpusMusic { skip_audio = true; }
                    if audio_data.data().len() <= 1 { skip_audio = true; }
                }
                match &item {
                    ...
                    StreamItem::BookEvents(events) => {
                        for ev in events {
                            match ev {
                                tsclientlib::events::Event::Message { target, invoker, message } => {
                    ...
                    StreamItem::Audio(packet) => {
                        let opus_bytes = packet.data().data().data();
                        let mut state = STATE.lock();
                        ...
                        match decode_result {
                            Some(Ok(Ok((samples, pcm_out)))) => {
                                ...
                                let peak = pcm_out[..safe_len].iter()
                                    .fold(0.0f32, |m, &s| m.max(s.abs()));
                                let buf_was = state.audio_out.len();
                                state.audio_out.extend_from_slice(&pcm_out[..safe_len]);
                                eprintln!("audio: decoded {} samp peak={:.4} buf {}->{}",
                                    safe_len, peak, buf_was, state.audio_out.len());
                                ...
                                if cnt % 25 == 1 {
                                    state.pending_events.push_back(TsEvent::AudioReceived {
                                        decoded: cnt, errors: errs, buf_samples: buf,
                                    });
                                }
                            }
                            Some(Ok(Err(e))) => {
                                state.audio_error_count += 1;
                                eprintln!("audio: decode err: {} hex={}",
                                    e, hex_slice(&opus_bytes[..opus_bytes.len().min(32)]));
                            }
                            Some(Err(panic_err)) => {
                                state.audio_error_count += 1;
            ...
            Ok(Some(Err(e))) => {
                ...
                let current_gen = crate::CONNECTION_GENERATION.load(Ordering::SeqCst);
                if current_gen == generation {
                    STATE.lock().pending_events.push_back(TsEvent::Error { message: format!("{}", e) });
                }
                break;
            }
            Ok(None) => {
                ...
                if current_gen == generation {
                    ...
                    s.connected = false;
                    s.pending_events.push_back(TsEvent::Disconnected { reason: "Connection closed by server".into() });
                    *COMMAND_TX.lock() = None;
                } else {
                    eprintln!("event_loop: stale stream-end ignored (current={}, my={})",
            ...
            if let Some(mut con) = crate::CONNECTION_STASH.lock().take() {
                ...
                for i in 0..10 {
                    let result = RUNTIME.block_on(tokio::time::timeout(
                        Duration::from_millis(200), con.events().next()));
                    match result {
                        Ok(Some(Ok(_))) => {},
                        Ok(Some(Err(_))) | Ok(None) => {
                        ...
                        Err(_) => {
                            if i == 9 { push_diag("ts_disconnect: direct quit timeout"); }
                        }
                    }
                }
            } else {
...
pub extern "C" fn ts_get_channels() -> *mut c_char {
    let state = STATE.lock();
    if !state.connected { return to_c_str("[]".to_string()); }
...
pub extern "C" fn ts_send_channel_message(_cid: u32, msg: *const c_char) -> u8 {
    ...
    if !STATE.lock().connected { return 0; }
    let tx = COMMAND_TX.lock();
    if let Some(tx) = tx.as_ref() {
        if tx.send(Command::SendMessage { target_mode: 2, target_cid: 0, message: msg }).is_ok() {
            1
        } else { 0 }
    } else { 0 }
}
...
pub extern "C" fn ts_move_to_channel(cid: u32) -> u8 {
    ...
    let tx = COMMAND_TX.lock();
    if let Some(tx) = tx.as_ref() {
        if tx.send(Command::MoveChannel { client_id: own_id as u16, channel_id: cid as u64 }).is_ok() {
            1
        } else { 0 }
    } else { 0 }
}
...
pub extern "C" fn ts_set_muted(inp: u8, out: u8) -> u8 {
    ...
    let tx = COMMAND_TX.lock();
    if let Some(tx) = tx.as_ref() {
        if tx.send(Command::SetMuted { input: inp != 0, output: out != 0 }).is_ok() {
            1
        } else { 0 }
    } else { 0 }
}
...
pub extern "C" fn ts_is_connected() -> u8 {
    if STATE.lock().connected { 1 } else { 0 }
}
...
pub extern "C" fn ts_start_audio() -> u8 {
    ...
    let encoder = match opus_rs::OpusEncoder::new(48000, 1, opus_rs::Application::Voip) {
        Ok(e) => e,
        Err(e) => { eprintln!("ts_start_audio: encoder error: {}", e); return 0; }
    };
    ...
    let decoder = match opus_rs::OpusDecoder::new(48000, 1) {
        Ok(d) => d,
        Err(e) => { eprintln!("ts_start_audio: decoder error: {}", e); return 0; }
    };
    let mut state = STATE.lock();
...
pub extern "C" fn ts_get_audio(buf: *mut f32, buf_len: u32) -> u32 {
    ...
    let peak = state.audio_out[..available].iter()
        .fold(0.0f32, |m, &s| m.max(s.abs()));
    unsafe {
        std::ptr::copy_nonoverlapping(state.audio_out.as_ptr(), buf, available);
    }
    state.audio_out.drain(..available);
    eprintln!("ts_get_audio: {} samp peak={:.4} rem={}",
        available, peak, state.audio_out.len());
    available as u32
}
...
pub extern "C" fn ts_send_audio(data: *const f32, data_len: u32) -> u8 {
    ...
    // Diagnostic: log PCM properties to debug garbled audio
    {
        let peak = samples.iter().fold(0.0f32, |m, &s| m.max(s.abs()));
        let rms = (samples.iter().map(|s| s * s).sum::<f32>() / samples.len() as f32).sqrt();
        let non_zero = samples.iter().filter(|&&s| s.abs() > 0.001).count();
        let first4: Vec<String> = samples.iter().take(4).map(|s| format!("{:.4}", s)).collect();
        eprintln!("ts_send_audio: {} samp peak={:.4} rms={:.4} non_zero={}/{} first=[{}]",
            samples.len(), peak, rms, non_zero, samples.len(), first4.join(", "));
    }
    let tx = COMMAND_TX.lock();
    if let Some(tx) = tx.as_ref() {
        if tx.send(Command::SendAudio { data: samples }).is_ok() { 1 } else { 0 }
    } else { 0 }
}
